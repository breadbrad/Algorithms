# For a DAG, memoized DP algorithm can be used directly 
...
def sp_dp(graph, v, result):
	...
	for u in graph.inverse(v): # Theta (indegree(v))
		new_distance = sq_dp(graph, u, result) + graph.weight(u, v)
		if new_distance < result.d[v]
			result.d[v] = new_distance
			result.parent[v] = u
		return result.d[v]
	# total running time of DP = # of subproblems * time per subproblem
	# |V| subproblems. total time = theta(E + V) (Handshaking Lemma)
